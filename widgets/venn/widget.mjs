...
export function render({ model, el }) {
  ...
  let highlight = model.get('highlight') || (sets === 3 ? 'ABC' : 'AB');
  ...
  function createSlider(label, value, testid, onInput) {
    ...
    valueSpan.className = 'sg-text-sm sg-font-medium sg-text-slate-700 dark:sg-text-stone-300';
  ...
  highlightGroup.className = 'sg-flex sg-items-center sg-gap-2.5';
  highlightGroup.style.marginBottom = '1rem';
  
  const highlightLabel = document.createElement('label');
  highlightLabel.className = 'sg-text-sm sg-font-medium sg-text-slate-700 dark:sg-text-stone-300';
  highlightLabel.textContent = 'Highlight:';
  
  const highlightSelect = document.createElement('select');
  highlightSelect.className = 'sg-px-3 sg-py-2 sg-text-sm sg-border sg-border-slate-300 dark:sg-border-stone-700 sg-rounded-md sg-bg-white dark:sg-bg-stone-900 sg-text-slate-900 dark:sg-text-stone-100 sg-shadow-sm sg-transition-colors hover:sg-border-slate-400 dark:hover:sg-border-stone-500 sg-focus:outline-none sg-focus:ring-2 sg-focus:ring-blue-500 sg-focus:border-blue-500';
  ...
  svgContainer.className = 'sg-border sg-border-slate-200 dark:sg-border-stone-700';
  svgContainer.style.borderRadius = '4px';
  svgContainer.className += ' sg-bg-white dark:sg-bg-stone-950';
  ...
  function computeGeometry() {
    ...
    const rC = sets === 3 ? C * Math.sqrt(pc) : 0;
  ...
  function updateProbDisplay() {
    if (sets === 3) {
      ...
      probDisplay.innerHTML = 
        `P(A) = ${pa.toFixed(3)} | P(B) = ${pb.toFixed(3)}<br>` +
  ...
  function renderVenn() {
    ...
    sBox.setAttribute('stroke', '#64748b');
    ...
    sLabel.setAttribute('fill', 'currentColor');
    ...
    labelA.setAttribute('x', geom.xA - (sets === 3 ? 0 : geom.rA * 0.6));
    labelA.setAttribute('y', geom.yA - (sets === 3 ? geom.rA * 0.6 : 0));
    labelA.setAttribute('text-anchor', 'middle');
    labelA.setAttribute('dominant-baseline', 'middle');
    labelA.style.fill = 'currentColor';
    ...
    labelB.setAttribute('x', geom.xB + (sets === 3 ? -geom.rB * 0.6 : geom.rB * 0.6));
    labelB.setAttribute('y', geom.yB + (sets === 3 ? geom.rB * 0.4 : 0));
    labelB.setAttribute('text-anchor', 'middle');
    labelB.setAttribute('dominant-baseline', 'middle');
    labelB.style.fill = 'currentColor';
    ...
    if (sets === 3) {
      ...
      labelC.style.fill = 'currentColor';
  ...
  function handleMouseDown(e) {
    ...
    if (sets === 3 && distC <= currentGeom.rC) { clicked = 'C'; minDist = distC; }
  ...
  function handleMouseMove(e) {
    if (!dragging || !currentGeom) {
      if (currentGeom) {
        ...
        if (distA <= currentGeom.rA || distB <= currentGeom.rB || (sets === 3 && distC <= currentGeom.rC)) {
